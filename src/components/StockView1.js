import React, { useState, useEffect } from "react";
import DataTable from "datatables.net-bs5"; // Import DataTables for Bootstrap 5
import "datatables.net-bs5/css/dataTables.bootstrap5.min.css"; // Import DataTables CSS
import "bootstrap/dist/css/bootstrap.min.css"; // Import Bootstrap CSS
import data from "../mockData/mockData.json";

function StockView1() {
  const [stock, setStock] = useState(data.stock);

  useEffect(() => {
    const fetchStock = async () => {
      const response = await fetch(
        "http://localhost:3001/api/category/stockview"
      );
      const data = await response.json();
      setStock(data);
    };
    fetchStock();
  }, []);

  useEffect(() => {
    const table = new DataTable("#stock-table", {
      destroy: true,
    });

    return () => {
      table.destroy();
    };
  }, [stock]);

  const calculateTotalValue = () => {
    return stock.reduce((sum, item) => sum + item.value, 0);
  };

  return (
    <div>
      <h2>Stock View</h2>
      <table id="stock-table" className="table table-striped">
        <thead>
          <tr>
            <th>Branch</th>
            <th>Asset Type</th>
            <th>Quantity</th>
            <th>Value</th>
          </tr>
        </thead>
        <tbody>
          {stock &&
            stock.map((item, index) => (
              <tr key={index}>
                <td>{item.branch}</td>
                <td>{item.type}</td>
                <td>{item.quantity}</td>
                <td>{item.value}</td>
              </tr>
            ))}
        </tbody>
        <tfoot>
          <tr>
            <td colSpan="3">
              <strong>Total Value:</strong>
            </td>
            <td>
              <strong>{calculateTotalValue()}</strong>
            </td>
          </tr>
        </tfoot>
      </table>
    </div>
  );
}

export default StockView1;
